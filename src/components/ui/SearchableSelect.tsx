...
export function SearchableSelect({
  ...
  return (
    <div className="relative min-w-0" ref={rootRef}>
      ...
      >
        <span
          className={`truncate ${selected || selectedLabel ? "font-semibold text-mendoza-text" : "text-mendoza-muted"}`}
        >
          {selectedLabel ?? selected?.label ?? allLabel}
        </span>
        <span className="flex shrink-0 items-center gap-1">
          {value && (
            <X
              aria-hidden="true"
              className="text-mendoza-muted opacity-0 transition group-hover:opacity-100"
              size={15}
            />
          )}
          <ChevronDown
      ...
      {open && (
        <div
          className={`absolute z-50 flex w-full min-w-64 flex-col overflow-hidden rounded-xl border border-mendoza-border bg-white shadow-xl shadow-slate-900/10 ${opensUpward ? "bottom-full mb-2" : "top-full mt-2"}`}
        ...
        >
              ...
              visibleOptions.map((option, index) => {
                ...
                return (
                  <button
                    ...
                    className={`flex w-full items-center gap-2 rounded-lg px-3 py-2.5 text-left text-sm transition ${index === activeIndex ? "bg-mendoza-sky/15" : "hover:bg-mendoza-background"}`}
                    id={`${listboxId}-${index}`}
          ...
          <p className="border-t border-mendoza-border px-3 py-2 text-xs text-mendoza-muted">
            {visibleOptions.length}{" "}
            {visibleOptions.length === 1 ? "opción" : "opciones"}
          </p>
        </div>
      )}
      {error && (
        <p
          className="mt-2 text-sm font-normal text-mendoza-error"
          id={errorId}
          role="alert"
        >
          {error}
