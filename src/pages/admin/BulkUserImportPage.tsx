...
export function BulkUserImportPage() {
  ...
  return <main className="p-4 sm:p-8"><div className="mx-auto max-w-6xl"><Link className="inline-flex items-center gap-2 text-sm font-semibold text-[#000F9F]" to="/admin/usuarios"><ArrowLeft size={17} />Volver a usuarios</Link><div className="mt-5 flex flex-wrap items-end justify-between gap-4"><div><p className="text-sm font-bold uppercase tracking-wide text-[#000F9F]">Carga masiva</p><h1 className="mt-1 text-3xl font-bold text-[#1F2937]">Importar usuarios</h1><p className="mt-2 text-[#6B7280]">Admite CSV o XLSX, hasta 500 filas y 2 MB.</p></div><Button icon={<Download size={17} />} onClick={() => void adminUsersService.downloadTemplate()} variant="outline">Descargar plantilla</Button></div>
    <section className="mt-6 rounded-2xl border border-[#E5E7EB] bg-white p-6 shadow-sm"><label className="flex cursor-pointer flex-col items-center rounded-xl border-2 border-dashed border-[#3CB4E5] bg-[#3CB4E5]/5 p-8 text-center"><FileSpreadsheet className="text-[#000F9F]" size={38} /><span className="mt-3 font-semibold text-[#1F2937]">{file?.name ?? 'Seleccionar archivo'}</span><span className="mt-1 text-sm text-[#6B7280]">CSV o Excel XLSX</span><input accept=".csv,.xlsx" className="sr-only" onChange={(event) => { setFile(event.target.files?.[0] ?? null); setPreview(null); setResult(null); }} type="file" /></label><div className="mt-4 flex justify-end"><Button disabled={!file || loading} icon={<Upload size={17} />} onClick={() => void previewFile()}>{loading ? 'Validando…' : 'Validar y previsualizar'}</Button></div></section>
    ...
    {result && <section className="mt-6 rounded-2xl border border-[#E5E7EB] bg-white p-6 shadow-sm"><h2 className="text-xl font-bold text-[#000F9F]">Resultado de importación</h2><p className="mt-2 text-[#1F2937]">Se importaron {result.importedCount} de {result.totalRows} filas. {result.errorCount} filas no fueron importadas.</p>{result.errors.length > 0 && <ul className="mt-4 space-y-2 text-sm text-[#DC2626]">{result.errors.map((error) => <li key={`${error.line}-${error.email}`}>Fila {error.line} ({error.email || 'sin correo'}): {error.errors.join(' ')}</li>)}</ul>}</section>}
  </div></main>;
}

function Metric({ label, value, positive, warning }: { label: string; value: number; positive?: boolean; warning?: boolean }) { return <div className="rounded-xl border border-[#E5E7EB] bg-white p-4"><p className="text-sm text-[#6B7280]">{label}</p><p className={`mt-1 text-2xl font-bold ${warning ? 'text-amber-700' : positive ? 'text-green-700' : 'text-[#000F9F]'}`}>{value}</p></div>; }

function PreviewTable({ preview }: { preview: ImportPreview }) { return <div className="mt-4 overflow-x-auto rounded-xl border border-[#E5E7EB] bg-white"><table className="min-w-full text-left text-sm"><thead className="bg-[#000F9F] text-white"><tr>{['Fila', 'Nombre', 'Correo', 'Rol', 'Colegio', 'Validación'].map((header) => <th className="px-3 py-3" key={header}>{header}</th>)}</tr></thead><tbody className="divide-y divide-[#E5E7EB]">{preview.rows.map((row) => <tr className={row.errors.length ? 'bg-red-50' : ''} key={row.line}><td className="px-3 py-3">{row.line}</td><td className="px-3 py-3">{row.lastName}, {row.firstName}</td><td className="px-3 py-3">{row.email || '—'}</td><td className="px-3 py-3">{row.role === 'admin' ? 'Administrador' : row.role === 'school' ? 'Colegio' : '—'}</td><td className="px-3 py-3">{row.schoolCode ?? '—'}</td><td className={`px-3 py-3 ${row.errors.length ? 'text-[#DC2626]' : 'font-semibold text-green-700'}`}>{row.errors.length ? row.errors.join(' ') : 'Fila válida'}</td></tr>)}</tbody></table></div>; }
