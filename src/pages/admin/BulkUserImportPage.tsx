...
export function BulkUserImportPage() {
  ...
  const previewFile = async () => {
    ...
    setResult(null);
    try {
      setPreview(await adminUsersService.preview(file));
    } catch (error) {
  ...
  const importRows = async () => {
    ...
    try {
      ...
      if (importResult.invitationEmailPendingCount > 0) {
        showWarning(
          `${importResult.importedCount} usuarios importados. ${importResult.invitationEmailPendingCount} correos no pudieron enviarse.`,
        );
      } else {
        showSuccess(
          `${importResult.importedCount} usuarios importados y notificados por correo.`,
        );
      }
      setPreview(null);
    } catch (error) {
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mt-5 flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <p className="mt-2 text-mendoza-muted">
              Admite CSV o XLSX, hasta 500 filas y 2 MB.
            </p>
          </div>
          <Button
            icon={<Download size={17} />}
            onClick={() => void adminUsersService.downloadTemplate()}
            variant="outline"
          >
            Descargar plantilla
          </Button>
        </div>
        <section className="mt-6 rounded-2xl border border-mendoza-border bg-white p-6 shadow-sm">
          <label className="flex cursor-pointer flex-col items-center rounded-xl border-2 border-dashed border-mendoza-sky bg-mendoza-sky/5 p-8 text-center">
            ...
            <span className="mt-3 font-semibold text-mendoza-text">
        ...
        {preview && (
          <section className="mt-6">
            <div className="grid gap-3 sm:grid-cols-3">
              ...
              <Metric label="Válidas" value={preview.validCount} positive />
              <Metric label="Con errores" value={preview.errorCount} warning />
            </div>
            {preview.errorCount > 0 && (
              <div className="mt-4 flex gap-3 rounded-xl border border-amber-300 bg-amber-50 p-4 text-sm text-amber-900">
                <AlertTriangle className="shrink-0" size={20} />
                <p>
                  La importación será parcial: se crearán únicamente las filas
                  válidas. Las filas con errores quedarán excluidas.
                </p>
              </div>
            )}
        ...
        {result && (
          <section className="mt-6 rounded-2xl border border-mendoza-border bg-white p-6 shadow-sm">
            ...
            <p className="mt-2 text-mendoza-text">
              Se importaron {result.importedCount} de {result.totalRows} filas.{" "}
              {result.errorCount} filas no fueron importadas.
            </p>
            <p className="mt-2 text-sm text-mendoza-muted">
              Se enviaron {result.invitationEmailSentCount} correos de acceso.{" "}
              {result.invitationEmailPendingCount > 0
                ? `${result.invitationEmailPendingCount} cuentas requieren entrega manual de credenciales.`
                : "Todas las cuentas creadas fueron notificadas."}
            </p>
            {result.errors.length > 0 && (
              <ul className="mt-4 space-y-2 text-sm text-mendoza-error">
                {result.errors.map((error) => (
                  <li key={`${error.line}-${error.email}`}>
                    Fila {error.line} ({error.email || "sin correo"}):{" "}
                    {error.errors.join(" ")}
                  </li>
                ))}
              </ul>
...
function Metric({
  ...
  return (
    <div className="rounded-xl border border-mendoza-border bg-white p-4">
      ...
      <p
        className={`mt-1 text-2xl font-bold ${warning ? "text-amber-700" : positive ? "text-green-700" : "text-mendoza-blue"}`}
      >
        {value}
      </p>
    </div>
    ...
    <div className="mt-4 overflow-x-auto rounded-xl border border-mendoza-border bg-white">
      <table className="min-w-full text-left text-sm">
        <thead className="bg-mendoza-blue text-white">
          <tr>
            {["Fila", "Nombre", "Correo", "Rol", "Escuela", "Validación"].map(
              (header) => (
        ...
        <tbody className="divide-y divide-mendoza-border">
          {preview.rows.map((row) => (
            <tr className={row.errors.length ? "bg-red-50" : ""} key={row.line}>
              ...
              <td className="px-3 py-3">{row.schoolCode ?? "—"}</td>
              <td
                className={`px-3 py-3 ${row.errors.length ? "text-mendoza-error" : "font-semibold text-green-700"}`}
              >
