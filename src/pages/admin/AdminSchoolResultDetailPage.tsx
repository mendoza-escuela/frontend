...
function Summary({ detail }: { detail: AdminSchoolResultDetail }) {
  if (!detail.submission) return <EmptyState icon={ClipboardList} title="Presentación no iniciada" description="La escuela no inició una presentación para esta campaña." />;
  ...
  return <div className="space-y-6">
    ...
    <Card><h2 className="font-bold text-mendoza-text">Trazabilidad del cálculo</h2><dl className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><Datum label="Identificador de resultado" value={result.id} /><Datum label="Cuestionario" value={result.survey ? `${result.survey.name} · v${result.survey.version.number}` : "No disponible"} /><Datum label="Algoritmo" value={result.calculation.algorithmVersion || "No disponible para este resultado"} /><Datum label="Fecha de cálculo" value={formatDateTime(result.calculation.calculatedAt)} /><Datum label="Esquema del snapshot" value={`v${result.calculation.snapshotSchemaVersion}`} /><Datum label="Origen del cálculo" value={result.calculation.source} /><Datum label="Ejecutado por" value={result.calculation.calculatedBy ? `${result.calculation.calculatedBy.firstName} ${result.calculation.calculatedBy.lastName}` : "Proceso automático o usuario no disponible"} /><Datum label="Fecha de envío" value={formatDateTime(detail.submission?.submittedAt)} /></dl></Card>
  </div>;
}
...
function Excluded({ detail }: { detail: AdminSchoolResultDetail }) { const excluded = detail.result?.excludedQuestions ?? []; return excluded.length ? <div className="space-y-3">{excluded.map((question) => <Card as="article" key={question.id}><p className="text-xs font-bold text-mendoza-muted">{question.code} · {question.dimension.title}</p><h2 className="mt-1 font-semibold">{question.prompt}</h2><p className="mt-3 text-sm"><strong>Motivo:</strong> {question.exclusion.reason}</p><JsonBlock label="Datos de la escuela usados por la regla" value={question.exclusion.relevantSchoolFacts} /><JsonBlock label="Reglas evaluadas" value={question.exclusion.rules} /></Card>)}</div> : <EmptyState title="No hay preguntas excluidas" description="El snapshot no registra exclusiones para este resultado." />; }
function Profile({ detail }: { detail: AdminSchoolResultDetail }) { return detail.historicalSchoolProfile ? <Card><h2 className="flex items-center gap-2 text-xl font-bold text-mendoza-text"><Archive size={20} /> Ficha usada al enviar</h2><p className="mt-1 text-sm text-mendoza-muted">Estos son los datos congelados de la escuela; no se sustituyen por su ficha actual.</p><dl className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{Object.entries(detail.historicalSchoolProfile).map(([key, value]) => <Datum key={key} label={profileLabel(key)} value={displayAnswer(value)} />)}</dl></Card> : <EmptyState icon={School} title="Ficha histórica no disponible" description="Esta presentación antigua no conserva una fotografía de la escuela." />; }
...
function JsonBlock({ label, value }: { label: string; value: unknown }) { return <details className="mt-3 text-sm"><summary className="cursor-pointer font-semibold text-mendoza-blue">{label}</summary><pre className="mt-2 overflow-auto rounded-lg bg-mendoza-background p-3 text-xs">{JSON.stringify(value, null, 2)}</pre></details>; }
...
function profileLabel(key: string) { return ({ name: "Nombre", cue: "CUE", directorName: "Director/a", address: "Domicilio", locality: "Localidad", scope: "Ámbito", educationLevel: "Nivel educativo", shift: "Jornada", hasKiosk: "Tiene kiosco", hasFoodService: "Tiene servicio alimentario", isBoarding: "Es albergue", enrollmentTotal: "Matrícula total", capturedAt: "Fecha de captura" } as Record<string, string>)[key] ?? key; }
